...
基于Akshare + Pandas + scikit-learn + Matplotlib的A股日线/分时数据处理工具。

脚本会为所配股票（示例：`600199 金种子酒`）自动：

* 拉取日线与分时数据

* 计算 20 日ATR、VWAP、开盘区间ORB

* 用KMeans聚类估算支撑/阻力及关键价位

* 判断交易风格（趋势/均值回归等）

* 生成图表并导出带使用说明的Excel仪表盘
...
* 对收盘价聚类，排序后的最小中心为支撑，最大中心为阻力，并标记最近关键价格。
