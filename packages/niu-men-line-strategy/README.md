...
2026-08-25 的扩展映射版本有 5183 只候选股票，3808 只满足至少 1008 根可用 bar 并完成
评估，1375 只因行业上下文预热后样本不足而跳过。按所有股票-窗口记录计算，NML 基线年化
收益率中位数为 -0.556%，简单价格趋势 gate 为 -0.425%，板块退潮过滤为 -0.482%。两种
过滤都减少入场次数并改善中位数最大回撤，但简单趋势 gate 的 Sharpe 中位数由 -0.335
降至 -0.384，暂时看不到稳定增量。高置信度版本评估 2229 只股票，基线、简单趋势 gate、
板块退潮过滤的年化收益率中位数分别为 -0.610%、-0.469%、-0.534%，方向一致。

这是数据覆盖、时点和过滤器联动的研究结果，买入持有是每个测试窗口内的全仓比较，不代表
组合级实盘结果。回测还记录了开盘涨跌停导致的无法成交，扩展映射版本基线被阻止买入 1237
次、阻止卖出 236 次。上述数字不能解读为策略已经获得正收益或具备稳定超额。
